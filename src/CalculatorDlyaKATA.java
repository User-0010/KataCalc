...
public class CalculatorDlyaKATA {
    public static String calc(String input) {
        //получение и перевод ввода данных пользователем в строку
...
//----------------Проверяю на наличие или отсутсвие знаков арифметики------------
        boolean f = false;
        int analiz2 = 0;
        for (char element : input.toCharArray()) {                                    // Проверка на присутсвие +-*/ и если нахожу,
            if (element == '-')
                analiz2++;                                                            // то плюсую единицу в analyz2, что бы использовать как "да" или "нет"
...
//--------------------------------------------------------------------------------
        ...
        int index9 = input.indexOf("+");      //ловим номер, на котором стоит знак
        int index10 = input.indexOf("-");
        int index11 = input.indexOf("*");
        int index12 = input.indexOf("/");


        char[] kantino = new char[10];          // создаем массив, что выловить правильный
        for (int i = 0; i < input.length(); i++) {     //делаем цикл, для заполнения массива
            kantino[i] = input.charAt(i);
        }            // глупо, но добываю символ операции, для его использования
        ...
        int index = input.indexOf(arifmet);               //просто с тестовой версии, решил включать все действия так, глупо, но я учусь
...
//_______________________________________________________
        ...
        if (index > 0) {      //при + ,index1 изменит свое значение на 1 и включится.
            ...
            String onestr1 = input.substring(0, index);  //разделим на первую половину выражения
            String twostr2 = input.substring(index + 1);// и вторую пловину
            ...
            for (int i = 0; i < onestr1.length(); i++) {     //делаем цикл, для заполнения массива
                onestr_char[i] = onestr1.charAt(i);
            }   //onestr1 - первая половина выражения в массив


            for (int i = 0; i < twostr2.length(); i++) {     //делаем цикл, для заполнения массива
                twostr_char[i] = twostr2.charAt(i);
            }   //twostr2 - вторая половина выражения в массив
...
//---------------------------------------------------------------------------------------------------------
            ...
            int analiz = 0;
            for (char element : input.toCharArray()) {                                                      // хотел реализовать что то подобное тому,
                if (element == '-')
                    analiz++;                                                           // что проверяет ввод пользователя, на наличие
                if (element == '+')
                    analiz++;                                                           // отрицательных чисел.
                if (element == '*') analiz++;
                if (element == '/') analiz++;
            }
...
//---------------------------------------------------------------------------------------------------------
            ...
            try {                                                                               //-Пришлось создать 2 try-catch блока, так как 2е исключение, не читалось
                ...
                //-
                    ...
                    ArrayIndexOutOfBoundsException e) {                                           // Это еще не дописанно, выходит ошибка по размеру массива
                System.out.println();
            }                                                                                   //-
// --------------------------------------------------------------------------------------------------------
            ...
            if (onestr_char[0] >= '1' && onestr_char[0] <= '9' && twostr_char[0] >= '1' && twostr_char[0] <= '9') { //Подумать, глупое решение. Проверка на цифры, через сравнение, по первым символам
                ...
                if (chislo1 >= 1 && chislo1 <= 10 && chislo2 >= 1 && chislo2 <= 10) { // проверка, цифры от 1 до 10.
                    return String.valueOf(Obrabotka2.poluchenie(chislo1, arifmet, chislo2));   // просим у класса Obrabotka2 обработать выражение и записываем его в otvet
                } else {
                ...
                if (chislo1 >= 1 & chislo1 <= 10 & chislo2 >= 1 & chislo2 <= 10) {
                    ...
                    if (otvet > 0) {
                        ...
                        return ANIR.convertation(otvet); // делаем конвертацию в классе ANIR и возвращаем готовый пирожок
                    } else {
